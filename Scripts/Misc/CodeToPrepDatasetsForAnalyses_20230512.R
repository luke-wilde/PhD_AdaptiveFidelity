...
newdata <- data.frame(expand.grid(km = seq(0,240,by = 1), fidc = c(1,2,3), Year = 2017, id_yr = "255_2018", AID = "255"))
...
ggplot(cbind(newdata, preddevsign)) + geom_ribbon(aes(x = km, ymin = (prediction - se), ymax = (prediction + se), group = factor(fidc)), fill = "#6A5ACD", alpha = .15) + geom_line(aes(x = km, y = (prediction), group = factor(fidc), linetype = factor(fidc)), linewidth = 1.4, color = "#4B0082") + scale_linetype_manual(values = c("solid", "dashed", "dotted"), labels = c("High", "Moderate", "Low")) + labs(x = "Route Distance (km)", y = "Days from Peak Green-up", linetype = "Fidelity") + theme_classic() + theme(axis.title.x = element_text(size = 20,color = "grey18"), axis.title.y = element_text(size = 20,color = "grey18"),axis.text.x = element_text(size = 17,color = "grey18"),axis.text.y = element_text(size = 17,color = "grey18"),legend.text = element_text(size = 16,color = "grey18"), title = element_text(size = 20,color = "grey18")) + coord_cartesian(ylim = c(-20,60), xlim = c(0,240))+ scale_y_continuous(expand = c(0,0), limits = c(-30,65.2), breaks = seq(-20,60,10)) + scale_x_continuous(expand = c(0,0), limits = c(0,255), breaks = seq(0,240,40)) + geom_hline(aes(yintercept = 0), linetype = "dashed", size = 1.2)

ggsave(filename = "Figures/OptimalDeviations_20230511.jpg", width = 30, height = 24, units = "cm", dpi = 600)
